...
interface Props {
  ...
  description?: string
  confirmLabel?: string
  /** 선택 가능한 최소 날짜(YYYY-MM-DD). 미지정 시 제한 없음 */
  min?: string
  /** 초기 선택 날짜(YYYY-MM-DD). 미지정 시 내일 */
  defaultDate?: string
}
...
export function DelayDialog({
  ...
  title = '날짜 선택',
  description = '이동할 날짜를 선택하세요',
  confirmLabel = '이동',
  min,
  defaultDate,
}: Props) {
  const [date, setDate] = useState(defaultDate ?? format(addDays(new Date(), 1), 'yyyy-MM-dd'))
